...
(function () {
  ...
  ReceiptGeneratorCore.prototype.generate = function (options, callbacks) {
    ...
    try {
      ...
      // SECURITY ENHANCEMENT: Add security hash validation after payment confirmation check
      if (opt.data_hash && opt.validation_secret) {
        ...
        if (!constantTimeCompare(opt.data_hash, expectedHash)) {
          throw new Error(
            "Security Error: Data consistency validation failed. Receipt generation aborted.",
          );
        }
        ...
        if (
          opt.timestamp &&
          Math.abs(Date.now() - parseInt(opt.timestamp)) > 300000
        ) {
          throw new Error(
            "Security Error: Timestamp validation failed. Request may be a replay attack.",
          );
        }
      }

      // ADDITIONAL SECURITY: Transaction ID validation
      if (opt.transaction_id) {
        // Prevent common injection patterns in transaction IDs
        if (/(<script|javascript:|on\w+\s*=|data:\s*text\/html)/i.test(opt.transaction_id)) {
          throw new Error('Security Error: Invalid characters detected in transaction ID.');
        }
        ...
        if (opt.transaction_id.length > 255) {
          throw new Error('Security Error: Transaction ID too long. Maximum 255 characters allowed.');
        }
      }

      var currencyCode =
        String(opt.currency_code || opt.currency || "USD")
          .trim()
          .toUpperCase() || "USD";
      ...
      var invNum = String(
        opt.invoice_number || opt.transaction_id || "RCP-001",
      );
      var invDate = String(
        opt.invoice_date ||
          opt.paid_date ||
      ...
      var sellerName = String(
        sellerInfo.name || sellerInfo.company_name || opt.seller_name || "",
      );
      var sellerAddr = String(
        sellerInfo.address || opt.seller_address || "",
      ).replace(/\\n/g, "\n");

      var buyerInfo = opt.buyer_info || {};
      var buyerName = String(buyerInfo.name || opt.buyer_name || "Customer");
      var buyerAddr = String(
        buyerInfo.address || opt.buyer_address || "",
      ).replace(/\\n/g, "\n");

      var notes = String(opt.notes || "");
      var filename = String(
        opt.filename ||
          "receipt-" + invNum.replace(/[^a-zA-Z0-9\-_]/g, "_") + ".pdf",
      ...
      debugLog(debug, "3.lineItems", {
        count: lineItems.length,
        calcedSubtotal: calcedSubtotal,
      });
      ...
      var grandTotal = round2(
        grandTotalRaw != null ? grandTotalRaw : subtotal + taxAmount,
      );
      debugLog(debug, "4.totals", {
        subtotal: subtotal,
        taxAmount: taxAmount,
      ...
      var y = 20,
        margin = isMobile ? 24 : 20;
      var pageW = doc.internal.pageSize.getWidth();
      var pageH = doc.internal.pageSize.getHeight();
      var colDesc = margin,
        colQty = isMobile ? 90 : 100,
        colPrice = isMobile ? 120 : 130,
        colAmt = pageW - margin - 30;
      ...
      doc.text(
        "Receipt #: " + invNum,
        pageW - margin - doc.getTextWidth("Receipt #: " + invNum),
        y,
      );
      y += 5;
      doc.text(
        "Date: " + invDate,
        pageW - margin - doc.getTextWidth("Date: " + invDate),
        y,
      );
      y += 5;
      if (transactionId && transactionId !== invNum)
        doc.text(
          "Transaction: " + transactionId,
          pageW - margin - doc.getTextWidth("Transaction: " + transactionId),
          y,
        );
      y += 5;
      if (paymentMethod)
        doc.text(
          "Paid: " + paymentMethod,
          pageW - margin - doc.getTextWidth("Paid: " + paymentMethod),
          y,
        );
      y += 5;
      doc.text(
        "Paid Date: " + paidDate,
        pageW - margin - doc.getTextWidth("Paid Date: " + paidDate),
        y,
      );
      doc.setFont(undefined, "normal");
      y += 12;
      ...
      if (sellerName || sellerAddr) {
        ...
        for (var s = 0; s < sellerLines.length; s++) {
          doc.text(sellerLines[s], margin, y);
          y += 5;
        }
        y += 5;
      }
      ...
      for (var b = 0; b < buyerLines.length; b++) {
        doc.text(buyerLines[b], margin, y);
        y += 5;
      }
      y += 12;
      ...
      // SECURITY: Obfuscate transaction ID in PDF metadata
      doc.setProperties({
      ...
      var base64String =
        dataUri.indexOf(",") >= 0 ? dataUri.split(",")[1] : dataUri;
      ...
      debugLog(debug, "6.base64Output", {
        len: base64String ? base64String.length : 0,
        hasDataUrl: !!dataUrl,
      });
