...
function initialize(instance, context) {
  if (typeof window.ReceiptGeneratorCore === "undefined") {
    instance.publishState(
      "last_error",
      "ReceiptGeneratorCore not loaded. Check Shared HTML Header.",
    );
    return;
  }
...
function buildReceiptOpts(def, p) {
  ...
  return {
    ...
    notes: p.notes || "",
    filename:
      p.filename && String(p.filename).trim() ? String(p.filename) : null,
    // SECURITY ENHANCEMENT: Add security parameters
    data_hash: p.data_hash || "",
    validation_secret: p.validation_secret || "",
    timestamp: p.timestamp || Date.now().toString(),
  };
}
...
function action_generate_receipt_pdf(instance, properties, context) {
  ...
  if (!opts.filename)
    opts.filename =
      "receipt-" +
      (opts.transaction_id || opts.invoice_number).replace(
        /[^a-zA-Z0-9\-_]/g,
        "_",
      ) +
      ".pdf";
    ...
    core.generate(opts, {
      ...
      onError: function (data) {
        ...
        if (errorMessage.includes("Security Error")) {
          console.error(
            "[ReceiptGenerator] Security validation failed:",
            errorMessage,
          );
          errorMessage =
            "Receipt generation failed due to security validation. Please contact support.";
        }
        instance.publishState("last_error", errorMessage);
...
// =========================================
// 1. TRANSACTION ID PROTECTION:
//    - Always pass transaction_id from server-side payment provider webhooks
//    - Never expose raw payment provider IDs in client-side code
//    - Use temporary reference IDs when possible
//
// 2. DATA VALIDATION:
//    - Use the security hash feature (data_hash + validation_secret)
//    - Validate all amounts server-side before receipt generation
//    - Implement timestamp validation to prevent replay attacks
//
// 3. SECURE DEPLOYMENT:
//    - Use HTTPS always
//    - Store validation_secret in Bubble's encrypted data storage
//    - Rotate secrets regularly in production
//
// 4. MONITORING:
//    - Monitor for security validation failures
//    - Log receipt generation events for audit trails
//    - Set up alerts for suspicious activity
//
// Security: Pass transaction_id and total from Stripe/payment provider, not user input.
//==============================================================================

function action_generate_and_upload_receipt(
  instance,
  properties,
  context,
  callback,
    ...
    core.generate(opts, {
      ...
      onSuccess: function (data) {
        ...
        if (!base64Data) {
          ...
          instance.triggerEvent("pdf_error");
          if (typeof callback === "function")
            callback({ success: false, error: "No base64 generated" });
          return;
        }
        if (typeof context.uploadContent !== "function") {
          var fallbackMsg =
            "uploadContent not available. Add plugin to app file storage or use Base64 File Uploader.";
        ...
        context.uploadContent(opts.filename, base64Data, function (err, url) {
          ...
          if (typeof callback === "function")
            callback({ success: true, file_url: url, filename: data.filename });
        });
      },
      onError: function (data) {
        // Handle security validation errors gracefully
